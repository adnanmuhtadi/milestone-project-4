...
class UserProfile(models.Model):
    ...
    with the user having a one to one relationship
    which specifics one user can only have one profile
    """
    ...
    default_county_state = models.CharField(
        max_length=80, null=True, blank=True)
...
def create_update_user_profile(sender, instance, created, **kwargs):
    """
    Create a profile if user has not got one or update the
    user profile if the user has a profile
    """
    if created:
