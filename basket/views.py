...
def add_to_basket(request, item_id):
    ...
    # Get the quantity from the basket and url to redirect
    ...
    if size:
        ...
        # Adding the product and quantity in the basket, and if the product
...
def remove_from_basket(request, item_id):
    ...
    # Try block to catch any expections that might
    # happen with a server 500 error
    try:
        product = get_object_or_404(Product, pk=item_id)
        basket = request.session.get('basket', {})
        basket.pop(item_id)
