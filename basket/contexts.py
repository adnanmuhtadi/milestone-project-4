...
def basket_contents(request):
    ...
    # For each item and quantity within the basket, get the
    # product then the basket and its calculations
    for item_id, item_data in basket.items():
        if isinstance(item_data, int):
            ...
            for size, quantity in item_data['items_by_size'].items():
                ...
                item_count += quantity
                # Add a dictionary to the list of basket items containing
                # the Item, quantity and product
                basket_products.append({
                    'item_id': item_id,
                    'quantity': quantity,
                    'product': product,
    ...
    # Adding items to the context which will available
    # to all templates across the site
    context = {
